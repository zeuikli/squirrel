...
本分支在原版鼠鬚管之上整合以下功能（macOS 13+）：
  ...
  * **語音輸入**：按住右 ⌥（Option）即說即上字，支援九種語言；後端可選（Gemini／Groq／ChatGPT），於偏好設定登入或填入 API key。中文辨識結果一律經本機 OpenCC 轉**台灣正體**，不依賴雲端潤飾是否成功；雲端潤飾（去填充詞／補標點／條列）失敗時會即時提示，並照常送出未潤飾的原稿（仍為繁體、不丟字）。
