#!/usr/bin/env python3
"""Add sources/Voice/*.swift to the Squirrel Xcode target.
...
PROJECT = os.path.join(os.path.dirname(__file__), "..", "Squirrel.xcodeproj", "project.pbxproj")
VOICE_DIR = os.path.join(os.path.dirname(__file__), "..", "sources", "Voice")
...
def main():
    ...
    voice_group = project.get_or_create_group("Voice", parent=sources_group)

    added, skipped = [], []
    for path in sorted(glob.glob(os.path.join(VOICE_DIR, "*.swift"))):
        rel = os.path.relpath(path, os.path.join(os.path.dirname(__file__), ".."))
        name = os.path.basename(rel)
        if project.get_files_by_name(name):
            skipped.append(name)
            continue
        refs = project.add_file(rel, parent=voice_group, target_name="Squirrel", force=False)
